...
export const pendingUsers=createAsyncThunk('auth/pendingusers',async(_,{rejectWithValue})=>{
  try {
    const response=await axiosInstance.get('auth/pendingusers',{ withCredentials: true });
    return response.data
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || 'Failed to get pending users');
  }
})
...
export const approveUserAction = createAsyncThunk("auth/approveuser", async (UserId, { rejectWithValue }) => {
  try {
    const response = await axiosInstance.put(`auth/approveuser/${UserId}`, {}, { withCredentials: true });
    return response.data;
  } catch (error) {
...
export const makeManagerAction = createAsyncThunk("auth/makemanager", async (UserId, { rejectWithValue }) => {
  try {
    const response = await axiosInstance.put(`auth/makemanager/${UserId}`, {}, { withCredentials: true });
    return response.data;
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || 'Failed to make user a manager');
  }
});

export const changeRoleAction = createAsyncThunk("auth/changerole", async ({ UserId, role }, { rejectWithValue }) => {
  try {
    const response = await axiosInstance.put(`auth/changerole/${UserId}`, { role }, { withCredentials: true });
    return response.data;
  } catch (error) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(pendingUsers.fulfilled, (state, action) => {
        state.pendinguser = Array.isArray(action.payload) ? action.payload : [];
      })
